...
class DescribeLaunch:
    ...
    def should_print_warning_when_vault_argument_provided(self, runner):
        with patch("zk_chat.commands.gui.create_default_console_gateway") as mock_create:
            with patch("zk_chat.qt.main"):
                runner.invoke(gui_app, ["launch", "--vault", "/some/vault"])

        mock_create.return_value.print.assert_any_call(
            "[yellow]Note:[/] Vault parameter (/some/vault) will be ignored."
        )

    def should_call_run_gui_when_launch_succeeds(self, runner):
        with patch("zk_chat.commands.gui.create_default_console_gateway"):
    ...
    def should_print_dependency_instructions_on_import_error(self, runner):
        ...
        calls = [str(call) for call in mock_create.return_value.print.call_args_list]

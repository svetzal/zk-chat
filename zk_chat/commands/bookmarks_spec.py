...
class DescribeListBookmarks:
    ...
    def should_load_config_from_gateway(self, mock_global_config_gateway, mock_console_gateway):
        mock_global_config_gateway.load.return_value = GlobalConfig()

        _list_bookmarks(mock_global_config_gateway, mock_console_gateway)

        mock_global_config_gateway.load.assert_called_once()

    def should_handle_empty_bookmarks(self, mock_global_config_gateway, mock_console_gateway):
        mock_global_config_gateway.load.return_value = GlobalConfig()
...
class DescribeRemoveBookmark:
    ...
    def should_return_true_when_bookmark_exists(self, mock_global_config_gateway, mock_console_gateway):
        config = GlobalConfig()
        abs_path = os.path.abspath("/some/vault")
        config.add_bookmark(abs_path)
        mock_global_config_gateway.load.return_value = config

        result = _remove_bookmark(abs_path, mock_global_config_gateway, mock_console_gateway)

        assert result is True
        mock_global_config_gateway.save.assert_called_once()

    def should_return_false_when_bookmark_not_found(self, mock_global_config_gateway, mock_console_gateway):
        mock_global_config_gateway.load.return_value = GlobalConfig()

        result = _remove_bookmark("/nonexistent/path", mock_global_config_gateway, mock_console_gateway)

        assert result is False
        mock_global_config_gateway.save.assert_not_called()

    def should_use_gateway_to_save_after_removal(self, mock_global_config_gateway, mock_console_gateway):
        config = GlobalConfig()

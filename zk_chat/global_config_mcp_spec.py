...
class DescribeGlobalConfigMCPServers:
    ...
    def should_add_stdio_mcp_server(self):
        ...
        server = MCPServerConfig(
            name="test-server", server_type=MCPServerType.STDIO, command="test-command", args=["--arg1", "value1"]
        )
    ...
    def should_add_http_mcp_server(self):
        config = GlobalConfig()
        server = MCPServerConfig(name="http-server", server_type=MCPServerType.HTTP, url="http://localhost:8080")
    ...
    def should_remove_mcp_server(self):
        config = GlobalConfig()
        server = MCPServerConfig(name="test-server", server_type=MCPServerType.STDIO, command="test-command")
    ...
    def should_list_all_mcp_servers(self):
        config = GlobalConfig()
        server1 = MCPServerConfig(name="server1", server_type=MCPServerType.STDIO, command="command1")
        server2 = MCPServerConfig(name="server2", server_type=MCPServerType.HTTP, url="http://localhost:8080")
    ...
    def should_update_existing_server(self):
        config = GlobalConfig()
        server1 = MCPServerConfig(name="test-server", server_type=MCPServerType.STDIO, command="old-command")
        server2 = MCPServerConfig(name="test-server", server_type=MCPServerType.STDIO, command="new-command")
...
class DescribeMCPServerConfig:
    ...
    def should_validate_stdio_server_requires_command(self):
        try:
            MCPServerConfig(name="test", server_type=MCPServerType.STDIO)
    ...
    def should_validate_http_server_requires_url(self):
        try:
            MCPServerConfig(name="test", server_type=MCPServerType.HTTP)
            raise AssertionError("Should have raised ValueError")
        except ValueError as e:
            assert "URL" in str(e)

    def should_create_stdio_server_with_command(self):
        server = MCPServerConfig(name="test", server_type=MCPServerType.STDIO, command="test-command")
    ...
    def should_create_http_server_with_url(self):
        server = MCPServerConfig(name="test", server_type=MCPServerType.HTTP, url="http://localhost:8080")
    ...
    def should_create_stdio_server_with_args(self):
        server = MCPServerConfig(
            name="test", server_type=MCPServerType.STDIO, command="test-command", args=["--flag", "value"]
        )

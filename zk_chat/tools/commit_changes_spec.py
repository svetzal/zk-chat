...
@pytest.fixture
def llm_broker(mock_ollama_gateway):
    """Real LLMBroker backed by a mocked OllamaGateway."""
    return LLMBroker(model="test", gateway=mock_ollama_gateway)


@pytest.fixture
def commit_changes(mock_git_gateway, llm_broker, mock_console_gateway):
    """Fixture for CommitChanges instance with real LLMBroker and mocked gateways."""
    return CommitChanges("/mock/path", llm_broker, mock_git_gateway, mock_console_gateway)
...
class DescribeCommitChanges:
    ...
    def should_be_instantiated_with_base_path_llm_and_git(self, llm_broker, mock_console_gateway):
        """Test that CommitChanges can be instantiated with a base path, LLM broker,
        and GitGateway."""
        base_path = "/test/path"
        mock_git = Mock(spec=GitGateway)

        tool = CommitChanges(base_path, llm_broker, mock_git, mock_console_gateway)
    ...
    def should_return_error_message_when_committing_fails(self, commit_changes, mock_git_gateway, mock_ollama_gateway):
        ...
        mock_ollama_gateway.complete.return_value = _response("Test commit message")
        ...
        mock_git_gateway.get_status.assert_called_once()
        mock_git_gateway.get_diff.assert_called_once()
        mock_ollama_gateway.complete.assert_called_once()
    ...
    def should_successfully_commit_changes(self, commit_changes, mock_git_gateway, mock_ollama_gateway):

...
- **Renamed documents stay searchable**: renaming a document now removes the old path's document and excerpt entries from the vector database and re-indexes the document under its new path, so a renamed note is findable by search immediately instead of only after a full reindex.
- **Repeated passages across notes stay searchable**: excerpt entries in the vector index are now keyed by the owning document path and the excerpt's ordinal position, not just its text. Previously, two notes containing an identical passage produced colliding excerpt IDs — only one note's excerpt survived in the index and the other became unfindable for that content. Re-index your vault (`zk-chat index`) to regenerate excerpt IDs under the new scheme.
